...
const TesterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!testerName.trim() || Object.keys(answers).length !== project.questions.length) {
      alert("Mohon isi Nama Tester dan jawab seluruh pertanyaan.");
    ...
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Nama Tester</label>
            <input 
              type="text" 
              value={testerName}
              onChange={(e) => setTesterName(e.target.value)}
              placeholder="Masukkan nama lengkap Anda"
              className="w-full md:w-1/2 px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-slate-50"
              required
            />
          </div>
              ...
              <div key={q.id} className="bg-slate-50 p-5 rounded-lg border border-slate-100">
                ...
                <div className="flex gap-4 flex-wrap">
                  {[1, 2, 3, 4, 5].map((score) => (
                    <label 
                      key={score} 
                      className={`flex flex-col items-center justify-center w-12 h-12 rounded-full cursor-pointer transition-all border-2 ${
                        answers[q.id] === score 
